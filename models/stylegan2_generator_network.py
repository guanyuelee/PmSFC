...
class StyleGAN2GeneratorNet(nn.Module):
  ...
  """
    ...
    def forward(self, z, which_block=0, pre_model=False, post_model=False):
        if pre_model:
            cur_layer = 0
            w = self.mapping(z, which_block=which_block, pre_model=pre_model, post_model=post_model)
            if which_block < self.mapping.num_layers:
                return w
            else:
                cur_layer += self.mapping.num_layers

            w = self.truncation(w)
            if cur_layer == which_block:
                return w
            cur_layer += 1

            x_patch = self.synthesis(w, which_block=which_block - cur_layer, pre_model=pre_model, post_model=post_model)
            return x_patch
            ...
            if isinstance(z, tuple):
                x = self.synthesis(z, which_block=which_block - cur_layer, pre_model=pre_model, post_model=post_model)
            else:
                x = self.synthesis(w, which_block=which_block - cur_layer, pre_model=pre_model, post_model=post_model)
            return x
...
class SynthesisModule(nn.Module):
  ...
  """
    ...
    def __init__(self,
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == self.init_res:
                ...
                print('Intialize %d layer.' % (2 * block_idx - 1))
                self.add_module(
            ...
            print('Intialize %d layer.' % (2 * block_idx))
            self.add_module(
                f'layer{2 * block_idx}',
                ModulateConvBlock(resolution=res,
    ...
    def forward(self, input_patch, which_block=0, pre_model=False, post_model=False, free_model=False, start=5, end=8):
        if pre_model:
            w = input_patch
        ...
        if post_model:
            if isinstance(input_patch, tuple):
                x, image, w = input_patch
            else:
                w = input_patch
                x = self.early_layer(w)
                image = None
