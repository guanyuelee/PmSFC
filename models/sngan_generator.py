...
__all__ = ['SNGANGenerator', 'BigGANShallowGenerator']
...
class SNGANGenerator(BaseGenerator):
    """Defines the generator class of PGGAN."""

    def __init__(self, model_name, logger=None):
        super().__init__(model_name, logger)
        assert self.gan_type == 'sngan'
    ...
    def build(self):
        ...
        config = Args(**json.load(open(self.config_path)))
        self.config = config
        ...
        gen_config = SN_RES_GEN_CONFIGS[config.model]
        generator = make_resnet_generator(gen_config, channels=image_channels, z_dim=config.latent_dim,
                                          img_size=MODELS[config.model])
