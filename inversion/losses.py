...
def get_mmd_loss(sigma_list):
    return MMDLoss(sigma_list)
...
class MMDLoss(nn.Module):
    ...
    def forward(self, x, y, biased=True):
        # calculate the mmd distance between x and y
        mmd = mix_rbf_mmd2(x, y, sigma_list=self.sigma_list, biased=biased)
        return mmd
...
class VGGLoss(nn.Module):
   ...
   (30): MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1, ceil_mode=False)
    """
    def __init__(self, vgg_layer, args, reduce=True):
    ...
    def forward(self, x, gt):
        ...
        return self.loss(x_features, gt_features, reduction='mean', reduce=self.reduce) * 0.001
